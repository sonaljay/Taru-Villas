...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    const existing = await getMenuCategoryById(id)
    if (!existing) {
    ...
    const updated = await updateMenuCategory(id, parsed.data)
    return NextResponse.json(updated)
  } catch (error) {
    console.error('PATCH /api/menus/categories/[id] error:', error)
    return NextResponse.json(
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  try {
    ...
    console.error('DELETE /api/menus/categories/[id] error:', error)
    return NextResponse.json(
