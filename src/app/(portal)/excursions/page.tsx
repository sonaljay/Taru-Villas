...
export default async function ExcursionsPickerPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Excursions</h1>
        <p className="text-sm text-muted-foreground">
          Select a property to manage its excursions
        </p>
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {properties.map((property) => {
          ...
          return (
            <Link
              key={property.id}
              href={`/properties/${property.id}/excursions`}
            >
