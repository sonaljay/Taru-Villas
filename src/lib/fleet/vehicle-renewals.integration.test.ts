  ...
  it('round trips metadata, deduplicates, reassigns, preserves history and rejects foreign assignments', async () => {
    ...
    try {
      await db.transaction(async tx => {
        ...
        await tx.update(tasks).set({ approval: 'approved', status: 'in_progress' }).where(eq(tasks.id, taskId))
        await reconcileVehicle(tx, id, manager.orgId, today)
        await reconcileVehicle(tx, id, manager.orgId, today)
        const [unchanged] = await tx.select().from(tasks).where(eq(tasks.id, taskId))
        expect(unchanged.approval).toBe('approved')
        ...
        const [completed] = await tx.select().from(tasks).where(eq(tasks.id, taskId))
