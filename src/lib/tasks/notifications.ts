   ...
   update task_notification_deliveries d set state='sending',lease_until=now()+interval '2 minutes',attempts=attempts+1 from pick where d.id=pick.id returning d.*`)
    ...
    try {
      await db.transaction(async (tx) => {
        // Hold authorization rows through delivery so revocation cannot race the send.
        await tx.execute(
          sql`select id from tasks where id=${job.task_id}::uuid for share`,
        )
        await tx.execute(
          sql`select profile_id from task_assignees where task_id=${job.task_id}::uuid for share`,
        )
        const a = await loadActor(String(job.profile_id), tx),
          task = await getWorkflowTask(a, String(job.task_id), false, tx)
        ...
        if (job.channel === 'in_app') {
          ...
          if (!origin || new URL(origin).protocol !== 'https:')
            throw Error(
              'Configure TASK_APP_ORIGIN with the deployed HTTPS URL.',
            )
